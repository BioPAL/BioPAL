...
def check_pm_kz0(kz_stack, opt_str):
    ...
    kz_list=[]; [kz_list.append(v) for v in kz_stack.values()]
    del kz_stack
    
    
    condition = np.nansum( (np.nanmax( kz_list, axis=0 ) >=  opt_str.kz0) & ( np.nanmin( kz_list, axis=0 ) <=  opt_str.kz0))  < \
...
def ground_cancellation_core(data_stack, pol_name, kz_stack, opt_str):
    ...
    if num_acq == 2:
        ...
        for acq_id in data_stack.keys():
            data_stack[acq_id][pol_name] = data_stack[acq_id][pol_name]*np.exp(-1j*kz_stack[acq_id]*opt_str.z_demod)
...
def ground_cancellation(data_stack, kz_stack, multi_master_flag, z_emph, eq_flag, pho_r, theta_look, ground_slope):
    ...
    if not eq_flag == '1' and not eq_flag == '2' and not eq_flag == '3':
        error_str = 'Configuration flag "ModelBasedEqualization" value "{}" not valid, choose among "1", "2" or "3", where "1"->always OFF; "2"->always ON; "3"->ON only if two acquisitions are present'.format(eq_flag)
    ...
    for pol_name in pol_names:
        ...
        if num_acq == 2:
            ...
            for acq_id in kz_stack.keys():
                ...
                print("For Master {} of pol {} the number of pixels that satisfy the criterion is {}".format(acq_id, pol_name, pixelcount))
        ...
        if eq_flag == '2': # 2 = always active
            ...
            if num_acq == 2:
                normalization_factor       = SingleBaselineEqualization(z_emph, pho_r, kz_stack[acq_names[1]], theta_look, ground_slope)
...
def SingleBaselineEqualization(eq_height, pho_r, kz, theta_look, ground_slope):
    ...
    P = 2*((xr0 + delta_xr/2)*(1 - np.sin(kxr*(xr0 + delta_xr/2))/(kxr*(xr0 + delta_xr/2))) +\
