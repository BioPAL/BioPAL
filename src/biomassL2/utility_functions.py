...
def tandemx_search_fnf_tiles( geographic_boundaries ):
    ...
    lat_tiles = ["S{:02d}".format(89-l) for l in range(0, 89, tile_extent_lat)] + ["N{:02d}".format(l) for l in range(0, 89, tile_extent_lat)]
    ...
    for lat_idx in np.arange(len(lat_start)):
        ...
        lon_tiles = ["W{:03d}".format(180-l) for l in range(0, 180, tile_extent_lon)] + ["E{:03d}".format(l) for l in range(0, 180, tile_extent_lon)]
        ...
        for lon_idx in np.arange(len(lon_start)):
            ...
            geotransform        = [ lon_start[lon_idx] ,  pixel_spacing_lon, 0.0, lat_start[lat_idx] + tile_extent_lat, 0.0, - pixel_spacing_lat ]
...
def tandemx_fnf_read( fnf_catalogue, geographic_boundaries ):
    ...
    for tile_idx in np.arange(len(fnf_string_list)):
        ...
        fnf_aux_inf_file_path = os.path.join(fnf_catalogue, fnf_string_list[tile_idx], 'AUXFILES', fnf_string_list[tile_idx] + '_INF.txt')
...
def tandemx_fnf_write(out_fnf_path, fnf_raster, lon_raster, lat_raster):
    ...
    fnf_string_list, geotransform_list, tile_extent_lonlat_list = tandemx_search_fnf_tiles(lon_raster_min, lon_raster_max, lat_raster_min, lat_raster_max)
    ...
    for tile_idx in np.arange(len(fnf_string_list)):
        ...
        lon_out = np.arange(geotransform_list[tile_idx][0], geotransform_list[tile_idx][0] + tile_extent_lonlat_list[tile_idx][0] + geotransform_list[tile_idx][1], geotransform_list[tile_idx][1])
        lat_out = np.arange(geotransform_list[tile_idx][3], geotransform_list[tile_idx][3] - tile_extent_lonlat_list[tile_idx][1] + geotransform_list[tile_idx][5], geotransform_list[tile_idx][5])
