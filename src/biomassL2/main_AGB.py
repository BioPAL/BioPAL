...
from biomassL2.processing_AGB          import tableLookupInt, compute_processing_blocs_order, forward, inverse, regularizeIndices, tableLookupFloat, interp2d_wrapper, \
                                              check_intersection, merge_agb_intermediate, mean_on_rois, initialize_inversion_parameters, get_projection_from_path
from biomassL2.utility_orchestrator    import read_and_oversample_data, read_and_oversample_aux_data, choose_equi7_sampling,check_if_path_exists, \
                                              geocoding_init, geocoding,tiff_formatter, fnf_equi7_load_filter_equi7format, fnf_tandemx_load_filter_equi7format,\
                                              apply_calibration_screens, apply_dem_flattening,         compute_and_oversample_geometry_auxiliaries, \
                                              check_if_geometry_auxiliaries_are_present, check_fnf_folder_format, check_cal_format, get_min_time_stamp_repository,\
                                              get_raster_cal_names, get_foss_cal_names, resolution_heading_correction
from biomassL2.IO_interfaces           import parse_chains_input_file, parse_chains_configuration_file, decode_unique_acquisition_id_string       
...
def main_AGB( input_file_xml, configuration_file_xml, geographic_boundaries, geographic_boundaries_per_stack, gdal_path ):
    ...
    equi7_sampling_intermediate = choose_equi7_sampling( proc_conf.AGB.intermediate_ground_averaging,  proc_conf.AGB.intermediate_ground_averaging/2 )
    ...
    if fnf_format == 'TANDEM-X':
        ...
        equi7_fnf_mask_fnames = fnf_tandemx_load_filter_equi7format ( proc_inputs.forest_mask_catalogue_folder, e7g_intermediate, proc_conf.AGB.intermediate_ground_averaging, temp_output_folder, gdal_path, geographic_boundaries, time_tag_mjd_initial )
        ...
        equi7_fnf_mask_fnames = fnf_equi7_load_filter_equi7format  ( proc_inputs.forest_mask_catalogue_folder, e7g_intermediate, proc_conf.AGB.intermediate_ground_averaging, temp_output_folder, gdal_path)
    ...
    for stack_progressive_idx, (unique_stack_id, acquisitions_pf_names) in enumerate( proc_inputs.stack_composition.items() ):

        global_cycle_idx, heading_deg, rg_swath_idx, rg_sub_swath_idx, az_swath_idx, _ = \
        decode_unique_acquisition_id_string( unique_stack_id+'_BSL_00')
        
        # LUT: progresive stacks
        lut_progressive_stacks[stack_progressive_idx, :] = [stack_progressive_idx, global_cycle_idx, heading_deg, rg_swath_idx, rg_sub_swath_idx, az_swath_idx]
        ...
        try:
            ...
            beta0_calibrated, master_id, raster_info, raster_info_orig = \
            read_and_oversample_data( proc_inputs.L1c_repository, acquisitions_pf_names, proc_conf.enable_resampling )
        ...
        try:
            ...
            geometry_aux_are_present = check_if_geometry_auxiliaries_are_present(proc_inputs, unique_stack_id, acquisitions_pf_names, read_ref_h = read_ref_h)
            ...
            if proc_conf.compute_geometry or not geometry_aux_are_present:
                ...
                if proc_conf.compute_geometry:
                    ...
                    logging.warning( 'AGB: calling geometry library since AuxiliaryProductsFolder "Geometry" is empty or not complete \n')
                
                _, _, ellipsoid_slope, _, _, _, sar_geometry_master = \
                compute_and_oversample_geometry_auxiliaries(proc_inputs.L1c_repository, proc_inputs, unique_stack_id, acquisitions_pf_names, master_id, proc_conf.enable_resampling, force_ellipsoid=True )                   
                                          
                ecef_grid, off_nadir_angle_rad, slope, kz, reference_height, _, _= \
                compute_and_oversample_geometry_auxiliaries(proc_inputs.L1c_repository, proc_inputs, unique_stack_id, acquisitions_pf_names, master_id, proc_conf.enable_resampling, comp_ref_h = read_ref_h, sar_geometry_master=sar_geometry_master)
                ...
                for swath_id in kz.keys():
                    kz[swath_id] = kz[swath_id]  * np.sin(off_nadir_angle_rad[master_id]) / np.sin(off_nadir_angle_rad[master_id] - ellipsoid_slope)
                ...
                logging.info( 'AGB: geometry auxiliaries are provided from user, so they are now being loaded and not computed, for stack '+unique_stack_id+'\n')
               
                ecef_grid, off_nadir_angle_rad, slope,  kz, reference_height, _, _, _, _, _, _, _ = \
                read_and_oversample_aux_data( proc_inputs, unique_stack_id, acquisitions_pf_names, proc_conf.enable_resampling, raster_info_orig, read_ref_h = read_ref_h )
            ...
            if read_cal_screens:
                ...
                _, _, _, _, _, _, _, cal_screens, cal_screens_raster_info, _, _, _ = \
                read_and_oversample_aux_data( proc_inputs, unique_stack_id, acquisitions_pf_names, proc_conf.enable_resampling, raster_info_orig, read_cal_screens = read_cal_screens,\
                                              read_ecef = False, read_off_nadir = False, read_slope = False, read_kz = False , read_ref_h = False, read_dist = False )
                logging.info('...done')
        ...
        try:
            if proc_conf.apply_calibration_screen:
                ...
                beta0_calibrated = apply_calibration_screens( beta0_calibrated, raster_info, cal_screens, cal_screens_raster_info, master_id)
                logging.info('...done.\n')
        ...
        try:
            ...
            DN_beta0_notched = ground_cancellation(beta0_calibrated, kz, proc_conf.ground_cancellation.multi_master_flag, proc_conf.ground_cancellation.enhanced_forest_height, \
                            proc_conf.ground_cancellation.equalization_flag, raster_info.resolution_m_slant_rg, off_nadir_angle_rad[master_id], slope) # beta0_calibrated: 3 pol (nrg x Naz  x Nimmagini); DN_beta0_notched: 3 pol (Nrg x Naz x 1 immagine) 
        ...
        if proc_conf.AGB.intermediate_ground_averaging > proc_conf.AGB.product_resolution/2:
            ...
            logging.warning( '"intermediate_ground_averaging" cannot be greater than "product_resolution/2", setting it to {}'.format(sigma_ground_res_m) )
        else:
        ...
        windtm_y = np.int( np.round( sigma_ground_res_m / (raster_info.pixel_spacing_slant_rg / np.sin(look_angle_rad)) / 2 ) * 2 + 1 )
    
        sub_factor_x = np.int( (windtm_x-1) / 2 ) 
        sub_factor_y = np.int( (windtm_y-1) / 2 )

        logging.info('AGB: multilooking of incidence angle...') 
        theta_multi_looked_sr = convolve2d( off_nadir_angle_rad[master_id] - slope, np.ones( (windtm_y, windtm_x) ) / windtm_y / windtm_x, mode='same') 
        ...
        for pol_name in DN_beta0_notched.keys():
            ...
            beta0_notched_multi_looked = convolve2d( np.absolute( DN_beta0_notched[pol_name] )**2 , np.ones( (windtm_y, windtm_x) ) / windtm_y / windtm_x, mode='same')
        
            logging.info('AGB: sigma0 computation for polarization {}...'.format(pol_name)) 
            sigma0_sr[pol_name] = beta0_notched_multi_looked[ ::sub_factor_y, ::sub_factor_x ]  * np.sin( theta_multi_looked_sr ) 
        ...
        try:
            ...
            min_spacing_m = min(sub_factor_x*raster_info.pixel_spacing_az, sub_factor_y*raster_info.pixel_spacing_slant_rg)
            min_spacing_m = min( min_spacing_m, equi7_sampling_intermediate )
            
            logging.info( 'Geocoding spacing set to {} [m]'.format(min_spacing_m)  )
            
            lon_in, lat_in, alt_in, lonMeshed_out, latMeshed_out, valid_values_mask, \
            lon_regular_vector, used_lon_step, lat_regular_vector, used_lat_step = \
            ...
            for pol_name in sigma0_sr.keys():
                ...
                sigma0_gr[pol_name] = geocoding( sigma0_sr[pol_name], lon_in, lat_in, lonMeshed_out, latMeshed_out, valid_values_mask )
            ...
            theta_multi_looked_gr = geocoding( theta_multi_looked_sr, lon_in, lat_in, lonMeshed_out, latMeshed_out, valid_values_mask )
            logging.info('...done.\n') 
        ...
        try:
            ...
            geotransform              = [ upper_left_easting_coord, sampling_step_east_west, 0, upper_left_northing_coord, 0, sampling_step_north_south ] 
            
            # geotiff of the sigma0 (three polarizations)
            sigma0_ground_fnames = {}
            for pol_name in sigma0_gr.keys():
                sigma0_ground_fnames[pol_name] = os.path.join( temp_output_folder_gr, 'sigma0_'+pol_name+'_'+unique_stack_id+'.tif')
                
                tiff_formatter( sigma0_gr[pol_name ], sigma0_ground_fnames[pol_name] , geotransform, gdal_data_format=gdal.GDT_Float32 )
        ...
        try:
            ...
            for pol_name in sigma0_ground_fnames.keys():
                ...
                sigma0_equi7_fnames[unique_stack_id][pol_name] = image2equi7grid( e7g_intermediate, sigma0_ground_fnames[pol_name],  equi7_sigma0_outdir, gdal_path=gdal_path, inband=None, subgrid_ids=None, accurate_boundary=False, resampling_type='bilinear', tile_nodata=np.nan)
            ...
            theta_equi7_fnames[unique_stack_id] = image2equi7grid( e7g_intermediate, theta_ground_fname,  equi7_theta_outdir, gdal_path=gdal_path, inband=None, subgrid_ids=None, accurate_boundary=False, resampling_type='bilinear', tile_nodata=np.nan)
        ...
        _, x_upper_left, y_upper_left = e7g_intermediate.lonlat2xy(geographic_boundaries_per_stack[unique_stack_id].lon_min, geographic_boundaries_per_stack[unique_stack_id].lat_max)
        north_in = y_upper_left
        east_min = x_upper_left
        
        # lower right corner for output map (UTM 32S)
        _, x_lower_right, y_lower_right = e7g_intermediate.lonlat2xy(geographic_boundaries_per_stack[unique_stack_id].lon_max, geographic_boundaries_per_stack[unique_stack_id].lat_min)
        north_out = y_lower_right
        east_max = x_lower_right
        
        lut_stacks_boundaries[stack_progressive_idx, :] = [east_min, east_max, min(north_in, north_out), max(north_in, north_out) ]
    ...
    dx_in, N0, E0, NN, EN, dE, dN, dE_roi, dN_roi, wE_roi, wN_roi, dE_par, dN_par, wE_par, wN_par, \
    all_vars, l_lims, a_lims, n_lims, w_lims, N_tests, geographic_grid_sampling, sub_grid_string = \
    initialize_inversion_parameters(equi7_sampling_intermediate, proc_inputs.geographic_grid_sampling, geographic_boundaries, proc_conf.AGB)
    ...
    acq_info_tab = np.column_stack((np.kron(np.arange(3),np.ones(acq_info_tab.shape[0])),np.kron(np.ones((3,1)),acq_info_tab)))

    # Compute the starting block and the nblock processing order
    par_block_index_curr, block_order = compute_processing_blocs_order( lut_cal, EE_par_coordinates, wE_par, NN_par_coordinates, wN_par )  
    ...
    while ~np.all(par_finished):
        ...
        par_extent_curr = np.array( [ EE_par_coordinates[par_block_index_curr], EE_par_coordinates[par_block_index_curr]+wE_par, NN_par_coordinates[par_block_index_curr], NN_par_coordinates[par_block_index_curr]+wN_par ] ) # [min_east, max_east, min_north, max_north]
        
        # Generate roi grid for current parameter block
        EE_roi_mesh, NN_roi_mesh = np.meshgrid( np.arange( par_extent_curr[0], par_extent_curr[1], dE_roi), np.arange( par_extent_curr[2], par_extent_curr[3], dN_roi) )
        ...
        if number_of_rois < proc_conf.AGB.min_number_of_rois:
          logging.info('skipping block #{} because the number of rois #{} cannot be less than #{}'.format(par_block_index_curr, number_of_rois, proc_conf.AGB.min_number_of_rois))
          # swap the flag
        ...
        try:
            ...
            for stack_idx, stack_path in enumerate( lut_stacks_paths ):
                ...
                block_has_data[stack_idx] = check_intersection( lut_stacks_boundaries[stack_idx, 0], lut_stacks_boundaries[stack_idx, 1], lut_stacks_boundaries[stack_idx, 2], lut_stacks_boundaries[stack_idx, 3],\
                                            par_extent_curr[0], par_extent_curr[1], par_extent_curr[3], par_extent_curr[2]   ) 

            block_has_cal = np.zeros( len(lut_cal_paths) , dtype = 'bool' )
            for cal_idx, cal_path in enumerate( lut_cal_paths ):
        
                # go ahead only if current parameter block is at least partially contained in the data stack:
                block_has_cal[cal_idx] = check_intersection( lut_cal[cal_idx, 0], lut_cal[cal_idx, 1], lut_cal[cal_idx, 2], lut_cal[cal_idx, 3],\
                                            par_extent_curr[0], par_extent_curr[1], par_extent_curr[3], par_extent_curr[2]   ) 
            ...
            acqid_tab = np.nan*np.zeros( (number_of_stacks_inside*len(pol_names), number_of_rois) ) # progressive stacks ids (replicates values for each roi)
            sigma_tab = np.nan*np.zeros( (number_of_stacks_inside*len(pol_names), number_of_rois) ) # sigma0 values for each roi and for each pol in each stack
            theta_tab = np.nan*np.zeros( (number_of_stacks_inside,                number_of_rois) ) # theta values for each roi in each stack: it is replicated after for matching the three polarizations shape
            ...
            for fnf_idx, fnf_path in enumerate( lut_fnf_paths ):
                ...
                fnf_is_inside = check_intersection( fnf_boundaries[0], fnf_boundaries[1], fnf_boundaries[2], fnf_boundaries[3],\
                                                       par_extent_curr[0],    par_extent_curr[1],    par_extent_curr[3],    par_extent_curr[2]   )                     
                if fnf_is_inside:

                    fnf_mask_data_interp_curr = np.round( interp2d_wrapper( fnf_path, 1, EE_pixel_axis, NN_pixel_axis, fill_value=float(0)) )
     
                    # mean all the fnf tiles
                    fnf_mask_data_interp = np.ceil( merge_agb_intermediate( fnf_mask_data_interp, fnf_mask_data_interp_curr, method = 'nan_mean') )
                ...
                if block_has_data[stack_idx]:
                    ...
                    for pol_name in pol_names:
                        ...
                        for equi7_tile_name in equi7_tiles_names:
                            ...
                            sigma0_tiff_name = os.path.join( sigma0_eq7_parent_dir_name, equi7_tile_name, 'sigma0_'+pol_name+'_'+os.path.basename(stack_path)+'_'+equi7_grid_name[6:]+'_'+equi7_tile_name+'.tif')
                            
                            stack_data_interp_curr = interp2d_wrapper( sigma0_tiff_name, 1, EE_pixel_axis, NN_pixel_axis, fill_value=np.NaN ) 
                            
                            stack_data_interp = merge_agb_intermediate( stack_data_interp, stack_data_interp_curr, method = 'nan_mean')
                        ...
                        # Mean on ROI
                        data_roi_means_vec = mean_on_rois( stack_data_interp, EE_pixel_mesh, NN_pixel_mesh, EE_roi_axis, dE_roi, NN_roi_axis, dN_roi, 'mean')
                    ...
                    for equi7_tile_name in equi7_tiles_names:
                        ...
                        theta_tiff_name = os.path.join( theta_eq7_parent_dir_name, equi7_tile_name, 'theta_'+os.path.basename(stack_path)+'_'+equi7_grid_name[6:]+'_'+equi7_tile_name+'.tif')
                    
                        stack_theta_interp_curr = interp2d_wrapper( theta_tiff_name, 1, EE_pixel_axis, NN_pixel_axis, fill_value=np.NaN ) 

                        #mean all the equi7 tiles
                        stack_theta_interp = merge_agb_intermediate( stack_theta_interp, stack_theta_interp_curr, method = 'nan_mean')
                    ...
                    # Mean on ROI
                    theta_roi_means_vec = mean_on_rois( stack_theta_interp, EE_pixel_mesh, NN_pixel_mesh, EE_roi_axis, dE_roi, NN_roi_axis, dN_roi, 'mean')
            ...
            polid_tab        = np.kron(np.ones((N_lay,1)),np.kron(np.array([np.arange(3)]).transpose(),np.ones((1,number_of_rois   ))))     
            polid_tab_pixels = np.kron(np.ones((N_lay,1)),np.kron(np.array([np.arange(3)]).transpose(),np.ones((1,number_of_pixels ))))    
             
            
            # save and delete all the pixel tabs:
            np.save( os.path.join( sigma_tab_pixel_folder, 'sigma_tab_pixels_block_{}'.format(par_block_index_curr) ),  sigma_tab_pixels ) 
            np.save( os.path.join( acq_tab_pixel_folder  , 'acq_tab_pixels_block_{}'  .format(par_block_index_curr) ),  acqid_tab_pixels ) 
            np.save( os.path.join( theta_tab_pixel_folder, 'theta_tab_pixels_block_{}'.format(par_block_index_curr) ),  theta_tab_pixels ) 
            np.save( os.path.join( polid_tab_pixel_folder, 'polid_tab_pixels_block_{}'.format(par_block_index_curr) ),  polid_tab_pixels ) 
            ...
            valid_acqs = np.isin(acq_info_tab[:,1],unique_acqs[acq_count>=proc_conf.AGB.min_number_of_rois_per_stack*3])
            # mask out invalid acquisitions (either too few measurements or there are nans in sigma or theta images)
            acqid_tab[(~np.isin(acqid_tab,acq_info_tab[valid_acqs,1])) | np.isnan(sigma_tab) | (sigma_tab<0) | np.isnan(theta_tab)] = -1
        ...
        par_id_luts = [np.unique(acq_info_tab[valid_acqs,:]*np.array([curr_var]),axis=0,return_inverse=True)[1] for curr_var in all_vars]
        ...
        par_tabs = [tableLookupInt(acq_info_tab[valid_acqs,:2],curr_lut,np.column_stack((polid_tab.flatten(),acqid_tab.flatten()))).reshape(acqid_tab.shape) for curr_lut in par_id_luts]
        ...
        for cal_idx, cal_path in enumerate( lut_cal_paths ):
            ...
            if  block_has_cal[cal_idx]:
                ...
                cal_std1_interp = np.sqrt( merge_agb_intermediate( cal_std1_interp**2, cal_std1_interp_curr**2, method = 'nan_mean') )
                cal_std2_interp = np.sqrt( merge_agb_intermediate( cal_std2_interp**2, cal_std2_interp_curr**2, method = 'nan_mean') )
                cal_std3_interp = np.sqrt( merge_agb_intermediate( cal_std3_interp**2, cal_std3_interp_curr**2, method = 'nan_mean') )
                
 
        # Mean on ROI
        cal_data_roi_means_vec =          mean_on_rois( cal_data_interp,    EE_pixel_mesh, NN_pixel_mesh, EE_roi_axis, dE_roi, NN_roi_axis, dN_roi, method = 'nan_mean')
        cal_std1_roi_means_vec = np.sqrt( mean_on_rois( cal_std1_interp**2, EE_pixel_mesh, NN_pixel_mesh, EE_roi_axis, dE_roi, NN_roi_axis, dN_roi, method = 'nan_mean') )
        cal_std2_roi_means_vec = np.sqrt( mean_on_rois( cal_std2_interp**2, EE_pixel_mesh, NN_pixel_mesh, EE_roi_axis, dE_roi, NN_roi_axis, dN_roi, method = 'nan_mean') )
        cal_std3_roi_means_vec = np.sqrt( mean_on_rois( cal_std3_interp**2, EE_pixel_mesh, NN_pixel_mesh, EE_roi_axis, dE_roi, NN_roi_axis, dN_roi, method = 'nan_mean') )
        ...
        # make sure we have enough data
        if not ((N_cal_sub>=proc_conf.AGB.min_number_of_cals_per_test) & (N_roi_sub>=proc_conf.AGB.min_number_of_rois_per_test)):
        ...
        cal_sets = all_cal_ids[np.unique(np.row_stack([np.sort(np.random.permutation(np.arange(len(all_cal_ids)))[:N_cal_sub]) for ii in np.arange(10000)]),axis=0)]
        cal_sets = cal_sets[np.random.permutation(np.arange(cal_sets.shape[0]))[:N_tests],:]
        roi_sets = all_roi_ids[np.unique(np.row_stack([np.sort(np.random.permutation(np.arange(len(all_roi_ids)))[:N_roi_sub]) for ii in np.arange(10000)]),axis=0)]
        ...
        for cal_set,roi_set in zip(cal_sets,roi_sets):
            ...
            w_cal = forward( np.maximum(1,cal_tab[curr_calmask]) ) + (std1_tab_db*eps1)[curr_calmask]+(std2_tab_db*eps2)[curr_calmask]+(std3_tab_db*eps3)[curr_calmask]
            ...
            i0_l_cal,i0_a_cal,i0_n_cal = [par_tab[curr_calmask]+par_offset for par_tab,par_offset in zip(par_tabs,par_offsets)]
            i0_l_roi,i0_a_roi,i0_n_roi = [par_tab[curr_roimask]+par_offset for par_tab,par_offset in zip(par_tabs,par_offsets)]
            ...
            X_roi = [np.column_stack(([np.float32(i_l_roi==ii) for ii in np.arange(N_par)])),
            ...
            X_cal = [np.column_stack(([np.float32(i_l_cal==ii) for ii in np.arange(N_par)])),
            ...
            def G(x):
                ...
                return 2*(1/N*np.nansum(np.array([A*D_l_roi]).transpose()*X_roi[0] +\
                                        np.array([A*D_a_roi*w_roi]).transpose()*X_roi[1] +\
                                        np.array([A*D_n_roi*c_roi]).transpose()*X_roi[2] +\
                                        np.array([A*D_w_roi*a_roi]).transpose()*X_roi[3],axis=0) +\
                          1/M*np.nansum(np.array([B*D_l_cal]).transpose()*X_cal[0] +\
                                        np.array([B*D_a_cal*w_cal]).transpose()*X_cal[1] +\
                                        np.array([B*D_n_cal*c_cal]).transpose()*X_cal[2],axis=0))
            ...
            curr_output_table[curr_roimask,4,i_test] = (s_roi-(n0+dn*np.sin(x[i_n_roi])**2)*c_roi-(l0+dl*np.sin(x[i_l_roi])**2))*\
                (a0+da*np.sin(x[i_a_roi])**2)
            curr_output_table[curr_calmask,4,i_test] = (s_cal-(n0+dn*np.sin(x[i_n_cal])**2)*c_cal-(l0+dl*np.sin(x[i_l_cal])**2))*\
        ...
        agbest_tab_eq = inverse(np.array([np.nansum(curr_output_table[ilay*3+np.arange(3),:,4,:],axis=0)/np.nansum(curr_output_table[ilay*3+np.arange(3),:,1,:]**2,axis=0) for ilay in np.int32(np.arange(N_lay))]))
        ...
        res_1 = forward(agbest_tab_eq)-forward(np.array([[cal_tab[0,:]]])*np.ones((N_tests,N_lay,1))).transpose([1,2,0])
        ...
        agbest_tab_eq = inverse((np.array([np.nansum(curr_output_table[ilay*3+np.arange(3),:,4,:],axis=0)/np.nansum(curr_output_table[ilay*3+np.arange(3),:,1,:]**2,axis=0) for ilay in np.int32(np.arange(N_lay))])-bias_1))
        # calculate biomass estimates averaged over triplets for each individual test using the equation
        agbest_tab_eqbar = inverse((np.nansum(curr_output_table[:,:,4,:],axis=0)/np.nansum(curr_output_table[:,:,1,:]**2,axis=0)-bias_1))
        ...
        agbest_tab_eqbar = inverse((np.nansum(curr_output_table[:,:,4,:],axis=0)/np.nansum(curr_output_table[:,:,1,:]**2,axis=0)-bias_1-np.array([bias_2]).transpose()*np.ones((1,N_tests))))
        # calculate the final biomass estimates by averaging over multiple tests
        agbest = inverse((np.nanmean(np.nansum(curr_output_table[:,:,4,:],axis=0)/np.nansum(curr_output_table[:,:,1,:]**2,axis=0),axis=1)-bias_1-bias_2))
        ...
        agbest =  inverse((np.nanmean(np.nansum(curr_output_table[:,:,4,:],axis=0)/np.nansum(curr_output_table[:,:,1,:]**2,axis=0),axis=1)-bias_1-bias_2-bias_3))
        ...
        # pre-calculate selection matrices
        X = [np.column_stack(([np.float32(i_l==ii) for ii in np.arange(N_par)])),
        ...
        def G(x):
            ...
            A = (l+a*w+n*c-s)
            return 2*(1/N*np.nansum(np.array([A*D_l]).transpose()*X[0] +\
                                    np.array([A*D_a*w]).transpose()*X[1] +\
                                    np.array([A*D_n*c]).transpose()*X[2],axis=0))
        ...
        parest = [l0+dl*np.sin(x[:par_offsets[1]])**2,
                  a0+da*np.sin(x[par_offsets[1]:par_offsets[2]])**2,
                  n0+dn*np.sin(x[par_offsets[2]:par_offsets[3]])**2]

        # create parameter look-up table
        parameter_lut = np.column_stack((np.array([par_block_index_curr,EE_par_coordinates[par_block_index_curr],NN_par_coordinates[par_block_index_curr],EE_par_coordinates[par_block_index_curr]+wE_par,NN_par_coordinates[par_block_index_curr]+wN_par])*np.ones((np.sum(valid_acqs),1)),acq_info_tab[valid_acqs,0:2],np.column_stack([parest[ipar][par_id_luts[ipar]] for ipar in np.arange(3)])))
        # save parameter 
       

        with open(par_path,'at') as file_par:
            for curr_row in parameter_lut:
                file_par.write('\n%d\t%.2f\t%.2f\t%.2f\t%.2f\t%d\t%d\t%.2f\t%.2f\t%.2f' % tuple([curr_val for curr_val in curr_row]))
            file_par.write('\n')
            file_par.close()            
        ...
        roi_vals = [  agbest            .reshape( len(NN_roi_axis), len(EE_roi_axis) ),
        ...
        cal_out_intermediate = os.path.join( temp_output_folder_agb_est,  'cal_estimated_from_block_{}'.format(par_block_index_curr) )
        
        curr_cal_path = tiff_formatter( roi_vals, cal_out_intermediate,  geotransform, multi_layers_tiff=True, gdal_data_format=gdal.GDT_Float32)
        
        lut_cal_paths.append(  curr_cal_path )
        
        sigma_tab = np.load( os.path.join( sigma_tab_pixel_folder, 'sigma_tab_pixels_block_{}.npy'.format(par_block_index_curr) ) )
        theta_tab = np.load( os.path.join( theta_tab_pixel_folder, 'theta_tab_pixels_block_{}.npy'.format(par_block_index_curr) ) )
        acqid_tab = np.load( os.path.join( acq_tab_pixel_folder,   'acq_tab_pixels_block_{}.npy'  .format(par_block_index_curr) ) )
        polid_tab = np.load( os.path.join( polid_tab_pixel_folder, 'polid_tab_pixels_block_{}.npy'.format(par_block_index_curr) ) )
                                                                                                        
        par_tabs = [tableLookupFloat(parameter_lut[:,5:7],parameter_lut[:,7+ipar],np.column_stack((polid_tab.flatten(),acqid_tab.flatten()))).reshape(polid_tab.shape) for ipar in np.arange(3)]
        agbest_tab = inverse((np.nansum((forward(sigma_tab)-par_tabs[0]-par_tabs[2]*forward(np.cos(theta_tab)))*par_tabs[1],axis=0)/\
        ...
        pix_vals = [ agbest_tab.reshape( len(NN_pixel_axis), len(EE_pixel_axis) ),
                    (agbest_tab*0+agbstd_1).reshape( len(NN_pixel_axis), len(EE_pixel_axis) ),
                    sp.interpolate.griddata((EE_roi_mesh[~np.isnan(agbstd_2)],NN_roi_mesh[~np.isnan(agbstd_2)]),agbstd_2[~np.isnan(agbstd_2)],(EE_pixel_mesh,NN_pixel_mesh),method='nearest')*(~np.isnan(agbest_tab.reshape( len(NN_pixel_axis), len(EE_pixel_axis) ))),
                    sp.interpolate.griddata((EE_roi_mesh[~np.isnan(agbstd_3)],NN_roi_mesh[~np.isnan(agbstd_3)]),agbstd_3[~np.isnan(agbstd_3)],(EE_pixel_mesh,NN_pixel_mesh),method='nearest')*(~np.isnan(agbest_tab.reshape( len(NN_pixel_axis), len(EE_pixel_axis) )))]

        quality_layer = np.sqrt( pix_vals[1]**2+pix_vals[2]**2+pix_vals[3]**2 )

        agb_est_ground_out_file_curr = os.path.join( temp_output_folder_agb_est, 'agb_est_block_{}.tif'.format(par_block_index_curr) )
        geotransform_curr = [ EE_pixel_axis[0], dE, 0, NN_pixel_axis[0], 0, dN ]
          
        agb_est_ground_out_file_curr = tiff_formatter( [pix_vals[0] , quality_layer], agb_est_ground_out_file_curr , geotransform_curr, gdal_data_format=gdal.GDT_Float32, projection=projection_curr, multi_layers_tiff=True )       
        ...
        try:
            ...
            logging.error( 'Cannot recognize input FNF Equi7 mask "{}" sub-grid folder name :'.format(subgrid_code)+str(e), exc_info=True )
            raise
                       
        bbox = [ (lon_min, lat_min), (lon_max, lat_max) ]    
        ftiles = e7g_product.search_tiles_in_roi(bbox=bbox)
  
        equi7_agb_est_outdir_curr = os.path.join( temp_output_folder_agb_est, 'eq7_agb_est_block_{}'.format(par_block_index_curr) )
        os.makedirs(equi7_agb_est_outdir_curr)
        equi7_agb_est_outdir_temp = image2equi7grid( e7g_product, agb_est_ground_out_file_curr,  equi7_agb_est_outdir_curr, gdal_path=gdal_path, ftiles=ftiles, accurate_boundary=False, tile_nodata=np.nan)
    ...
    for (tile_name, equi7_agb_est_out_tif_names_curr_tile) in equi7_agb_est_out_tif_names_per_tile.items():
        ...
        for idx_tile, equi7_agb_est_out_tif_name_curr_tile in enumerate( equi7_agb_est_out_tif_names_curr_tile ):
            ...
            quality_merged = np.sqrt( np.nanmean( np.dstack( ( quality_merged**2, driver.GetRasterBand(2).ReadAsArray()**2 ) ) ,axis=2 ) )
            driver = None    
        
        invalid_values_mask = np.logical_or( data_merged< proc_conf.AGB.estimation_valid_values_limits[0], data_merged> proc_conf.AGB.estimation_valid_values_limits[-1] )
        data_merged   [ invalid_values_mask ]= np.nan
        quality_merged[ invalid_values_mask ]= np.nan
        
        sub_grid_folder_name = os.path.basename( os.path.dirname( os.path.dirname( equi7_agb_est_out_tif_names_curr_tile[0] ) ) )
        output_folder_curr_tile = os.path.join( output_folder, sub_grid_folder_name, tile_name, 'agb_est_'+ tile_name+sub_grid_folder_name+'.tif')
        if not os.path.exists( os.path.dirname(output_folder_curr_tile) ):
            os.makedirs( os.path.dirname(output_folder_curr_tile) )
        output_folder_curr_tile = tiff_formatter( [data_merged, quality_merged], output_folder_curr_tile , geotransform_out, gdal_data_format=gdal.GDT_Float32, projection=projection_curr, multi_layers_tiff = True )
