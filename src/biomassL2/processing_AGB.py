...
def initialize_inversion_parameters(eq7_sampling, geographic_grid_sampling, geographic_boundaries, proc_confAGB):
    ...
    sub_grid_string, x_upper_left, y_upper_left = e7g_intermediate.lonlat2xy(geographic_boundaries.lon_min, geographic_boundaries.lat_max)
    N0 = y_upper_left
    E0 = x_upper_left
    
    # lower right corner for output map (UTM 32S)
    _, x_lower_right, y_lower_right = e7g_intermediate.lonlat2xy(geographic_boundaries.lon_max, geographic_boundaries.lat_min)
    NN = y_lower_right
    EN = x_lower_right
    ...
    if geographic_grid_sampling > (proc_confAGB.product_resolution / 2):
        ...
        logging.warning( 'Input "geographic_grid_sampling" cannot be greater than configuration "product_resolution"/2, setting to {}'.format(geographic_grid_sampling) )
    else:
    ...
    wE_par =  np.abs( proc_confAGB.parameter_block_size ) # dE_par*2 # 3/2 times spacing means that each tile shares 33.3% with its neighbour
    wN_par = -np.abs( proc_confAGB.parameter_block_size )#dN_par*2 # 3/2 times spacing means that each tile shares 33.3% with its neighbour
    ...
    all_vars = [np.array([var_pol.agb_model_scaling  , var_stack.agb_model_scaling  , var_gc.agb_model_scaling  , var_h.agb_model_scaling  , var_sw.agb_model_scaling  , var_subsw.agb_model_scaling  , var_az.agb_model_scaling  ]),
                np.array([var_pol.agb_model_exponent , var_stack.agb_model_exponent , var_gc.agb_model_exponent , var_h.agb_model_exponent , var_sw.agb_model_exponent , var_subsw.agb_model_exponent , var_az.agb_model_exponent ]),
                np.array([var_pol.agb_cosine_exponent, var_stack.agb_cosine_exponent, var_gc.agb_cosine_exponent, var_h.agb_cosine_exponent, var_sw.agb_cosine_exponent, var_subsw.agb_cosine_exponent, var_az.agb_cosine_exponent])]
    ...
    dN_roi = -dE_roi
    logging.info('ROI ground pixel spacing (east and north) adjusted from {} to {} [m] in order to be an intere multiple of output map ground pixel spacing '.format(dE_roi_old, dE_roi) )

    # adjust PAR spacings to be intere multiples of ROI ground pixel spacing
    dE_par_old = dE_par
    dE_par = round( dE_par / dE_roi ) * dE_roi
    dN_par = -dE_par
    logging.info('PAR ground pixel spacing (east and north) adjusted from {} to {} [m] in order to be an intere multiple of ROI pixel spacing '.format(dE_par_old, dE_par) )

    return dx_in, N0, E0, NN, EN, dE, dN, dE_roi, dN_roi, wE_roi, wN_roi, dE_par, dN_par, wE_par, wN_par, \
...
def compute_processing_blocs_order( lut_cal, EE_par_coordinates, wE_par, NN_par_coordinates, wN_par ):
    ...
    intersection_areas = compute_intersection_area( EE_par_coordinates, EE_par_coordinates+wE_par, NN_par_coordinates+wN_par, NN_par_coordinates, largest_cal_coords[0], largest_cal_coords[1], largest_cal_coords[2], largest_cal_coords[3] )
    ...
    block_order = np.argsort( np.sqrt((EE_par_coordinates-EE_par_coordinates[par_block_index_curr])**2+(NN_par_coordinates-NN_par_coordinates[par_block_index_curr])**2) ) # tile_order
...
def regularizeIndices(a,b,off):
    ...
    f = sp.interpolate.interp1d(np.concatenate((-np.ones(1),lut[:,0])),np.concatenate((-np.ones(1),lut[:,1])),kind='nearest')
...
def check_intersection( A_east_min, A_east_max, A_north_min, A_north_max, B_east_min, B_east_max, B_north_min, B_north_max ):
 
    if (A_east_min >= A_east_max).any() or (A_north_min >= A_north_max).any() or (B_east_min >= B_east_max).any() or (B_north_min >= B_north_max).any() :
        raise ValueError('Wrong inputs, at least one of the "min" variable is greater than (or equal to) the "max" one.')
        
    A_intersects_B = ( A_east_min  <= B_east_max  ) & ( A_east_max  >= B_east_min  )  & \
                     ( A_north_min <= B_north_max ) & ( A_north_max >= B_north_min )   
                    
    return A_intersects_B


def compute_intersection_area( A_east_min_vec, A_east_max_vec, A_north_min_vec, A_north_max_vec, B_east_min, B_east_max, B_north_min, B_north_max ):
   
    B_coords = tuple( zip([B_north_min, B_north_min, B_north_max, B_north_max],[B_east_min, B_east_max, B_east_min, B_east_max]) )
    ...
    for area_idx in np.arange(num_areas):
        A_coords = tuple( zip([A_north_min_vec[area_idx], A_north_min_vec[area_idx], A_north_max_vec[area_idx], A_north_max_vec[area_idx]],[A_east_min_vec[area_idx], A_east_max_vec[area_idx], A_east_min_vec[area_idx], A_east_max_vec[area_idx]]) )
...
def merge_agb_intermediate( a_data, b_data, method='nan_mean'):
    ...
    if method == 'nan_mean':
        ...
        a_data[ np.logical_and( b_data_is_present, a_data_is_present )]                     = np.mean( a_data[ np.logical_and( b_data_is_present, a_data_is_present )], b_data[ np.logical_and( b_data_is_present, a_data_is_present )] )
        
        # where only b is present perform the sum
        a_data[ np.logical_and( b_data_is_present, np.logical_not( a_data_is_present)   ) ] = np.sum ( a_data[ np.logical_and( b_data_is_present,  np.logical_not( a_data_is_present ) )], b_data[ np.logical_and( b_data_is_present,  np.logical_not( a_data_is_present ) )] )
