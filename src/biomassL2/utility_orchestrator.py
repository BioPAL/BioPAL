...
from biomassL2.IO_interfaces             import readBiomassHeader_core, getBinaryNameFromChannelIDX, input_params, write_chains_input_file, raster_info, geographic_boundaries, decode_unique_acquisition_id_string
from biomassL2.data_import               import read_data, read_ecef_grid, read_auxiliary_multi_channels, read_auxiliary_single_channel, data_oversample, get_data_time_stamp
from biomassL2.utility_functions         import epsg_in_to_epsg_out, tandemx_fnf_read
from biomassL2.constants                 import MAX_EARTH_RADIUS_METERS, OVERSAMPLING_FACTOR,EPSG_CODE_ECEF, EPSG_CODE_LLA, SATELLITE_VELOCITY
...
#NOTSET 0
    ...
    logging.basicConfig( handlers=[ logging.FileHandler( log_file_name, mode='w', encoding ='utf-8' ), logging.StreamHandler()], level=level_to_set, format='%(asctime)s - %(levelname)s | %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
...
def get_auxiliary_paths( stack_composition, L1c_aux_data_repository ):
    ...
    return ECEF_grid_file_names, kz_file_names, off_nadir_angle_file_names, reference_height_file_names, slant_range_distances_file_names, \
    slope_file_names, forest_mask_catalogue_folder, dem_folder, reference_agb_folder, average_covariance_folder,system_decorrelation_fun_folder,  \
    calibration_screens_folder , forest_height_folder, calibration_screens_file_names   
...
def data_select_by_date_and_boundaries( main_input_struct ):
    ...
    for pf_idx, pf_name in enumerate(pf_names_list):
        ...
        if len(pf_name) != 47:
            error_msg = 'Unrecognized uniqie acquisition ID (it will be skipped): "{}" from repository {}; the correct format is "GC_XX_H_XXX.XX_RGSW_XX_RGSBSW_X_AZSW_XX_BSL_XX" where each "X" is an integer'.format( pf_name , main_input_struct.L1c_repository)
        ...
        if not os.path.exists( raster_file ):
            logging.warning('The folder "'+bin_file_name+'" inside L1C repository is NOT a product folder and it will be skipped \n')
            continue
        ...
        if pf_idx == 0:
            edge_values = {'date_min':date, 'date_max': date, 'lon_min': lon_min, 'lon_max': lon_max, 'lat_min': lat_min, 'lat_max': lat_max}
        ...
        if uniqie_stack_id in stacks_geographic_boundaries.keys():
            ...
            stacks_geographic_boundaries[uniqie_stack_id].lon_min = min(stacks_geographic_boundaries[uniqie_stack_id].lon_min, lon_min)
            stacks_geographic_boundaries[uniqie_stack_id].lon_max = max(stacks_geographic_boundaries[uniqie_stack_id].lon_max, lon_max)
            stacks_geographic_boundaries[uniqie_stack_id].lat_min = min(stacks_geographic_boundaries[uniqie_stack_id].lat_min, lat_min)
            stacks_geographic_boundaries[uniqie_stack_id].lat_max = max(stacks_geographic_boundaries[uniqie_stack_id].lat_max, lat_max)
        else:
        ...
        boundaries_are_matching = check_on_boundaries(lon_min, lon_max, lat_min, lat_max, main_input_struct.geographic_boundary.point)
    ...
    if num_stacks == 0:
        ...
        if len(stacks_composition_only_dates_match) or len(stacks_composition_only_boundaries_match):
            ...
            if len(stacks_composition_only_dates_match):
                logging.warning('    Following stacks are confined in the desired L1cDates but not in the desired geographic boundaries')
                logging.warning('    {} \n'.format( list( stacks_composition_only_dates_match.keys() ) ))

            if len(stacks_composition_only_boundaries_match):
                logging.warning('    Following stacks are confined in the desired geographic boundaries but not in the desired L1cDates')
                logging.warning('    {} \n'.format( list( stacks_composition_only_boundaries_match.keys() ) ))
                
        if len(stacks_composition_not_enough_acq): 
            logging.warning('Following stacks contain just ONE acquisition (a minimum of TWO are needed to have ONE baseline)')
            logging.warning('{} \n'.format( list( stacks_composition_not_enough_acq.keys() ) ))
            
        if not len(stacks_composition_only_dates_match) and not len(stacks_composition_only_boundaries_match) and not len(stacks_composition_not_enough_acq):  
...
def write_chains_input_file_main( output_folder, main_input_struct, stack_composition ):
    ...
    ECEF_grid_file_names, kz_file_names, off_nadir_angle_file_names, reference_height_file_names, slant_range_distances_file_names, \
    slope_file_names, forest_mask_catalogue_folder, dem_folder, reference_agb_folder, average_covariance_folder,system_decorrelation_fun_folder,  \
    calibration_screens_folder , forest_height_folder, calibration_screens_file_names = \
    ...
    if main_input_struct.proc_flags.TOMO_FH:
        ...
        if len_stacks_tomo:
            if len_stacks_tomo < len_stacks_all:
                logging.info('The stacks with less than #3 acquisitions cannot be used to perform TOMO FH estimation \
                and they will be removed: #{} stacks among #{} totaL, have been removed'.format(len_stacks_tomo, len_stacks_all))
...
def read_and_oversample_data( L1c_repository, acquisitions_pf_names, enable_resampling ):
    ...
    for pf_name in acquisitions_pf_names:
        ...
        beta0_calibrated[pf_name], num_samples, num_lines, pixel_spacing_slant_rg, pixel_spacing_az, resolution_m_slant_rg, resolution_m_az, master_id, lines_start_utc = \
    ...
    raster_info_orig = raster_info ( num_samples,
                                     num_lines,
    ...
    if enable_resampling:
        ...
        beta0_calibrated, num_samples, pixel_spacing_slant_rg, num_lines, pixel_spacing_az = \
    ...
    raster_info_os = raster_info (  num_samples,
                                    num_lines,
                                    pixel_spacing_slant_rg,
                                    pixel_spacing_az,
                                    resolution_m_slant_rg,
...
def check_if_geometry_auxiliaries_are_present(file_names, stack_id, acquisitions_pf_names,
                                     read_ecef = True, read_off_nadir = True, read_slope = True, read_kz = True , read_ref_h = True, read_dist = True):
...
def read_and_oversample_aux_data( file_names, stack_id, acquisitions_pf_names, enable_resampling, raster_info,
                                  read_ecef  = True,  read_off_nadir     = True,  read_slope       = True,  read_kz = True , read_ref_h = True, read_dist  = True,
                                  read_average_cov   = False, read_cal_screens = False, read_FH = False, \
                                  read_reference_agb = False, read_sys_dec_fun = False ):
    ...
    if read_ecef:
        ...
        if not ecef_grid is None:
            ...
            logging.warning('Since "ECEFGRID/'+stack_id+'" folder is missing into AuxiliaryProductsFolder, geometric library needs to be called.\n')
    ...
    if read_kz:
        ...
        if not kz is None:
            ...
            logging.warning('Since "KZ/'+stack_id+'" folder is missing into AuxiliaryProductsFolder, geometric library needs to be called.\n')
    ...
    if read_off_nadir:
        ...
        if not off_nadir_angle_rad is None:
            ...
            logging.warning('Since "OffNadirAngles/'+stack_id+'" folder is missing into AuxiliaryProductsFolder, geometric library needs to be called.\n')
        
        
    if read_ref_h:
        logging.info('Loading auxiliary data: reference height...') # not needed, use just for comparison with the estimation
        pf_name          = os.path.basename              ( file_names.reference_height_file_names[stack_id] )
        folder_name      = os.path.dirname               ( file_names.reference_height_file_names[stack_id] )
        reference_height = read_auxiliary_single_channel ( folder_name, pf_name ) # it is the dtm in slant_range-azimuth reference    
        ...
        if not reference_height is None:
            ...
            logging.warning('Since "ReferenceHeight/'+stack_id+'" folder is missing into AuxiliaryProductsFolder, geometric library needs to be called.\n')
    ...
    if read_dist:
        ...
        if not R is None:
            ...
            logging.warning('Since "SlantRangeDistances/'+stack_id+'" folder is missing into AuxiliaryProductsFolder, geometric library needs to be called.\n')
    ...
    if read_slope:
        ...
        if not slope is None:
            ...
            logging.warning('Since "Slopes/'+stack_id+'" folder is missing into AuxiliaryProductsFolder, geometric library needs to be called.\n')
    ...
    if read_cal_screens:
        ...
        calibration_screens, cal_screens_raster_info = read_auxiliary_multi_channels ( folder, pf_name, acquisitions_pf_names, read_raster_info=True )
    ...
    return ecef_grid, off_nadir_angle_rad, slope, kz, reference_height, R, average_covariance, calibration_screens, cal_screens_raster_info, \
           forest_height, reference_agb, system_decorr_fun


def evaluate_estimation_quality_matrix( data_in_shape ):
    # Placemark for the quality estimation to be defined
    
    logging.warning( 'The quality estimation of the product is still to be defined: zeros will be placed in the quality layer ' )
...
def geocoding_init( ecef_grid, rg_vec_subs, az_vec_subs, min_spacing_m ):
    ...
    lon_in, lat_in, alt_in = epsg_in_to_epsg_out(ecef_grid['X'][rg_vec_subs, :][:, az_vec_subs] , ecef_grid['Y'][rg_vec_subs, :][:, az_vec_subs], ecef_grid['Z'][rg_vec_subs, :][:, az_vec_subs], EPSG_CODE_ECEF, EPSG_CODE_LLA)
    ...
    return lon_in, lat_in, alt_in, lonMeshed_out, latMeshed_out, valid_values_mask, lon_regular_vector, used_lon_step, lat_regular_vector, used_lat_step



def geocoding( data_to_geocode, lon_in, lat_in, lonMeshed_out, latMeshed_out, valid_values_mask, interp_method='linear'):
    f = griddata( np.array( [ lat_in[valid_values_mask].flatten(), lon_in[valid_values_mask].flatten() ] ).T , data_to_geocode[valid_values_mask].flatten() , np.array( [ latMeshed_out.flatten(), lonMeshed_out.flatten() ] ).T  , method=interp_method) # interpolate to fill holes
    data_ground = f.reshape(latMeshed_out.shape)
    
    return data_ground
  
   
def tiff_formatter( data_in, out_fname, geotransform, gdal_data_format, projection=None, multi_layers_tiff=False, time_tag=None ):
...
def fnf_tandemx_load_filter_equi7format ( forest_mask_catalogue_folder, e7g, product_resolution, output_folder, gdal_path, geographic_boundaries, time_tag_mjd_initial, dummy_mask_flag=False):
    ...
    fnf_tile_loaded_list, time_tag_mjd_tandemx, geotransform_list = tandemx_fnf_read( forest_mask_catalogue_folder, geographic_boundaries )
    ...
    if time_tag_mjd_tandemx > time_tag_mjd_initial:
        error_msg = 'Input FNF Mask (TANDEM-X) date of "{}" is bigger than input stack data minimum date of "{}"'.format( str(time_tag_mjd_tandemx), str(time_tag_mjd_initial))
        logging.error( error_msg )
    ...
    fnf_mask_ground_dir_names = \
    tiff_formatter( fnf_mask_list, fnf_mask_ground_dir_name, geotransform_list, gdal_data_format=gdal.GDT_Float32, multi_layers_tiff=False, time_tag=str( time_tag_mjd_tandemx ) )

    # conversion step 3: special case for the equi7 convertion of all the fnf masks found
    equi7_fnf_mask_tempdir_tiles_not_merged = [] # thos will contain all the fnf names in equi7: all fnf tiles, all equi7 tiles
    for idx, fnf_name_curr in enumerate( fnf_mask_ground_dir_names):
        ...
        equi7_out_name_curr = image2equi7grid( e7g, fnf_name_curr, equi7_fnf_mask_parent_tempdir,    gdal_path=gdal_path, inband=None, subgrid_ids=None, accurate_boundary=False, resampling_type='bilinear', tile_nodata=np.float(0))
    ...
    equi7_fnf_mask_fnames = fnf_equi7_masks_merging ( equi7_fnf_mask_tempdir_tiles_not_merged, equi7_fnf_mask_parent_tempdir )
...
def fnf_equi7_load_filter_equi7format( forest_mask_catalogue_folder, e7g_curr_chain, product_resolution, output_folder, gdal_path):
    ...
    for idx, equi7_fnf_mask_tiff_name in enumerate( equi7_fnf_mask_tiff_names_list ):
        ...
        intermediate_filtered_tiff_name = os.path.join( equi7_fnf_mask_parent_tempdir, 'initial_fnf_mask_equi7_{}.tif'.format(idx) )

        intermediate_filtered_tiff_name = tiff_formatter( fnf_filtered, intermediate_filtered_tiff_name , geotransform, gdal_data_format=gdal.GDT_Float32, projection=projection_equi7 )
        ...
        try:
            ...
            logging.error( 'Input FNF Equi7 mask geotransform is not in the correct format :'.format(subgrid_code)+str(e), exc_info=True )
            raise    
        ...
        out_equi7_folder_done = image2equi7grid( e7g_curr_chain, intermediate_filtered_tiff_name,  equi7_fnf_mask_parent_dir, gdal_path=gdal_path, ftiles=ftiles, accurate_boundary=False, tile_nodata=np.nan)
...
def fnf_equi7_masks_merging( fnf_mask_equi7_fnames_whole, temp_output_folder ):
    ...
    for fnf_tile_name in fnf_mask_equi7_fnames_whole: # fnf_mask_equi7_fnames is a list of lists: fnf_mask_equi7_fnames[ list of fnf tiles ][ list of equi7 tiles ]
    ...
    for equi7_tile_name in equi7_tile_names:
        ...
        out_fnf_equi7_name = os.path.join( temp_output_folder, 'equi7_fnf_mask_tiles_merged', equi7_subgrid_name, equi7_tile_name, '_fnf_mask_'+equi7_subgrid_name+'_'+equi7_tile_name+'.tif' )
        ...
        for fnf_tiff_full_path_curr in fnf_mask_equi7_fnames_whole:
            if equi7_tile_name in fnf_tiff_full_path_curr:
                ...
                if not 'data_sum' in locals():
                    ...
                    data_sum[where_both_are_not_zero      ] = ( loaded_equi7_tile[where_both_are_not_zero] + data_sum[where_both_are_not_zero] )/2
...
def fnf_and_validity_masks_merging( merged_fnf_equi7_names , validity_mask_equi7_fnames, temp_output_folder, stack_id):
    ...
    # retrive the names of all the used "EQUI7 tiles" and store in the equi7_tile_names: take the names from the generated EQUI7 folders for the fnf_mask_equi7 
    for fnf_equi7_name in merged_fnf_equi7_names: # fnf_mask_equi7_fnames is a list of lists: fnf_mask_equi7_fnames[ list of fnf tiles ][ list of equi7 tiles ]
    ...
    for equi7_tile_name in equi7_tile_names:
        ...
        if fnf_mask_name and validity_mask_name:
            ...
            out_final_equi7_name = os.path.join( temp_output_folder, 'equi7_global_mask', stack_id, equi7_subgrid_name, equi7_tile_name, stack_id+'_final_mask_'+equi7_subgrid_name+'_'+equi7_tile_name+'.tif' )
...
def mosaiking( equi7_main_full_folder, mosaiking_out_folder ):
    ...
    for equi7_stack_folder_name in equi7_stack_folder_names:
        ...
        equi7_subgrid_full_folder = os.path.join( equi7_main_full_folder, equi7_stack_folder_name, equi7_subgrid_folder_name )
    ...
    for equi7_tile_name in equi7_tile_names:
        ...
        for equi7_stack_folder_name in equi7_stack_folder_names:
            ...
            equi7_subgrid_folder    = os.path.join( equi7_main_full_folder, equi7_stack_folder_name, equi7_subgrid_folder_name )
            ...
            for equi7_tile_name_curr in equi7_tile_folder_names:
                ...
                if equi7_tile_name_curr == equi7_tile_name:
                    ...
                    equi7_tile_tiff_full_names.append( os.path.join( equi7_subgrid_folder, equi7_tile_name, equi7_tile_tiff_name ) )
        ...
        for equi7_tiff_name in equi7_tile_tiff_full_names:
            ...
            if not 'out_mosaiked_data' in locals():
                ...
                out_mosaiked_data[ where_both_are_not_zero       ] = ( loaded_equi7_tile_data[ where_both_are_not_zero      ] + out_mosaiked_data[where_both_are_not_zero] )/2
                
                out_mosaiked_quality[ where_loaded_only_is_not_zero ] = loaded_equi7_tile_quality  [ where_loaded_only_is_not_zero]
                out_mosaiked_quality[ where_both_are_not_zero       ] = ( loaded_equi7_tile_quality[ where_both_are_not_zero      ] + out_mosaiked_quality[where_both_are_not_zero] )/2
        
        # write the output final mosaiked tiff of the current equi7 tile:
        out_mosaiked_equi7_name = os.path.join( out_tile_equi7_full_folder, 'FH_'+equi7_subgrid_folder_name+'_'+equi7_tile_name+'.tif' )
...
def apply_calibration_screens( beta0_calibrated, beta0_raster_info, cal_screens, screens_raster_info, master_id):
    ...
    for idx, pf_name in enumerate( beta0_calibrated.keys() ):
        ...
        for pol_id in beta0_calibrated[pf_name] :
            beta0_calibrated[pf_name][pol_id] = np.multiply( beta0_calibrated[pf_name][pol_id], np.exp( -1j * cal_scree_interp ) )
...
def apply_dem_flattening( beta0_calibrated, kz_in, reference_height, master_id, raster_info)  :
    ...
    for pf_name in beta0_calibrated.keys():
        for pol_id in beta0_calibrated[pf_name] :
            beta0_calibrated[pf_name][pol_id] = np.multiply( beta0_calibrated[pf_name][pol_id], np.exp( -1j * np.multiply( kz_in[pf_name], reference_height ) ) )
...
def compute_and_oversample_geometry_auxiliaries( L1c_repository, aux_file_names, stack_id, acquisition_pf_names, master_id, enable_resampling, \
                                                comp_ref_h = True, comp_dist=True, force_ellipsoid=False, sar_geometry_master=None ):
    ecef_grid           = None
    off_nadir_angle_rad = None
    kz                  = None
    reference_height    = None
    ...
    if force_ellipsoid:
        logging.info('Geometry library: the ellipsoid altitude has been forced by input flag, only the slope over ellipsoid will be computed, after used to correcting geometry global / local reference.')
        dem = 0*dem
    ...
    sv_master          = geometric_lib.cut_state_vectors(ch_master.get_state_vectors(0),
    ...
    ecef_grid = {'X': sar_geometry_master.x_sar_coords, 'Y': sar_geometry_master.y_sar_coords, 'Z': sar_geometry_master.z_sar_coords}
    ...
    targets_coords = np.vstack((sar_geometry_master.x_sar_coords.flatten(),
    ...
    if comp_dist:
        ...
        for channel_idx, channel_curr in enumerate( ch_list ):
            ...
            if channel_idx == 0: 
                ...
                sv_slave = geometric_lib.cut_state_vectors(channel_curr.get_state_vectors(0),
                                                       ri_slave.lines_start,
                                                       ri_slave.lines_start + ri_slave.lines * ri_slave.lines_step)
                
                gso_slave = create_general_sar_orbit(sv_slave)
                
                _, range_points_master_slave = \
                    geometric_lib.perform_inverse_geocoding(gso_slave, targets_coords)
            ...
            if enable_resampling:
                R[ swath_id_list[channel_idx] ]  = data_oversample ( R[ swath_id_list[channel_idx] ] , OVERSAMPLING_FACTOR, ri_master )[0]  
    ...
    for channel_idx, channel_curr in enumerate( ch_list ):
        ...
        if channel_idx == 0: 
            ...
            off_nadir_angle_rad[ swath_id_list[channel_idx] ]  = sar_geometry_master.compute_off_nadir_angles_from_positions(sensor_positions_master, sensor_positions_master)
        else:
            # slaves
            ri_temp = channel_curr.get_raster_info(0)
            if ri_temp.samples_step_unit == 'm':
            ...
            off_nadir_angle_rad[ swath_id_list[channel_idx] ]  = sar_geometry_master.compute_off_nadir_angles_from_positions(sensor_positions_master, sensor_positions_slave)
                                                                                     
        if enable_resampling:
            off_nadir_angle_rad[ swath_id_list[channel_idx] ]  = data_oversample ( off_nadir_angle_rad[ swath_id_list[channel_idx] ] , OVERSAMPLING_FACTOR, ri_master )[0]    
    ...
    for channel_idx, channel_curr in enumerate( ch_list ):
        ...
        if channel_idx == 0:
            ...
            kz[ swath_id_list[channel_idx] ] = geometric_lib.compute_vertical_wavenumber_angles(wavelength, off_nadir_angle_rad[ swath_id_list[0] ], off_nadir_angle_rad[ swath_id_list[channel_idx]] )
         
        
        if enable_resampling:
            kz[ swath_id_list[channel_idx] ]  = data_oversample ( kz[ swath_id_list[channel_idx] ] , OVERSAMPLING_FACTOR, ri_master )[0]   
...
def get_equi7_fnf_tiff_names( fnf_mask_catalogue_folder):
    ...
    for idx, equi7_subgrid_name  in enumerate( os.listdir( fnf_mask_catalogue_folder ) ):
        ...
        if idx == 1:
            logging.warning('Found more than one sub-grid in input Equi7 format FNF mask: prototype supports just one grid at a time, only {} sub-grid will be read'.format(equi7_subgrid_name))
            break
        ...
        if e7g_type in temp_list[0][-2:]:
            ...
            logging.warning( 'Auxiliary forest mask multiple tiles sampling are not supported, only "'+e7g_type+'" tile sampling have been read, "'+temp_list[0][-2:]+'" skipped instead'  )
...
def get_min_time_stamp_repository( L1c_repository, stack_composition ):
    ...
    for idx, (unique_stack_id, unique_acq_pf_names) in enumerate( stack_composition.items() ): 
        ...
        if idx == 0:
            ...
            time_tag_mjd_initial = min( time_tag_mjd_initial, get_min_time_stamp_repository_core( L1c_repository, unique_acq_pf_names ))
...
def collect_stacks_to_be_merged( stack_composition ):
    ...
    for unique_stack_id in stack_composition.keys():
        ...
        global_cycle_idx, heading_deg, rg_swath_idx, rg_sub_swath_idx, az_swath_idx, baseline_idx = decode_unique_acquisition_id_string( stack_composition[unique_stack_id][0], output_format = 'string' )
        
        unique_merged_stack_id = 'GC_'+global_cycle_idx+'_RGSW_'+rg_swath_idx+'_RGSBSW_'+rg_sub_swath_idx+'_AZSW_'+az_swath_idx
...
def check_equi7_mask_coverage( data_equi7_fnames, equi7_fnf_mask_fnames):
    ...
    for equi7_tile_name in equi7_tile_names:
        if not equi7_tile_name in input_mask_equi7_tile_names:
            error_msg = 'The auxiliary "ForestMask" does not cover the acquisitions, missing Equi7 tile "{}"'.format( equi7_tile_name )
            logging.error(error_msg)
            raise ValueError(error_msg)
...
def convert_rasterinfo_meters_to_seconds( ri_meters ):
    ...
    ri_seconds = RasterInfo( ri_meters.lines, ri_meters.samples, ri_meters.cell_type, ri_meters.file_name, ri_meters.header_offset_bytes, ri_meters.row_prefix_bytes, ri_meters.byte_order)
...
def convert_rasterinfo_seconds_to_meters( ri_seconds ):
    ...
    ri_meters = RasterInfo( ri_seconds.lines, ri_seconds.samples, ri_seconds.cell_type, ri_seconds.file_name, ri_seconds.header_offset_bytes, ri_seconds.row_prefix_bytes, ri_seconds.byte_order)
