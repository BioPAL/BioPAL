...
geographic_boundaries = namedlist('geographic_boundaries', 
                    'lon_min \
                     lon_max \
                     lat_min \
                     lat_max' )
...
raster_info = namedtuple('raster_info', 
'num_samples \
 num_lines \
...
main_input_params = namedtuple('main_input_params', 
'L1c_repository \
...
proc_flags = namedlist('proc_flags_params', 
                        'AGB \
                         FH \
                         TOMO_FH \
                         FD \
...
L1c_date = namedtuple('L1c_date_params', 
                      'start \
                       stop')

# main_input_params "L1c_date" sub-fields:
geographic_boundary = namedtuple('geographic_boundary_params', 
                      'point')

# Input parameters structure (namedtuple)
input_params = namedtuple('input_params', 
'chain_id \
 L1c_repository \
 stack_composition \
...
configuration_params = namedtuple('configuration_params',
'chain_id \
 interpolate_stack \
 ground_cancellation \
 vertical_range \
...
agb_est_params     = namedtuple('agb_est_params', 
                                'number_of_tests \
...
agb_model_params = namedtuple('agb_model_params', 'agb_model_scaling_limits agb_model_exponent_limits agb_cosine_exponent_limits agb_estimation_limits \
...
FH_est_params     = namedtuple('FH_est_params', 
...
FH_model_params = namedtuple('FH_model_params', 'estimation_valid_values_limits maximum_height number_of_extinction_value number_of_ground_volume_ratio_value number_of_temporal_decorrelation_value')

# configuration_params "FD" sub-fields:
FD_est_params     = namedtuple('FD_est_params', 
                               'product_resolution \
                                confidence_level')

# configuration_params "TOMO FH" sub-fields:
TOMO_FH_est_params = namedtuple('TOMO_FH_est_params',
                                'estimation_valid_values_limits \
                                 product_resolution \
...
def writeBiomassHeader(product_folder, channel_idx, date_str, lon_min, lon_max, lat_min, lat_max, unique_acq_id ):
    ...
    packed_data        = struct.pack(_HEADER_FORMAT, encoded_date_str, lon_min, lon_max, lat_min, lat_max, encoded_unique_acq_id )

    raster_info_read = product_folder.get_channel( channel_idx ).metadata.get_metadata_channels(0).get_element('RasterInfo')
...
def readBiomassHeader_core( raster_file ):
    ...
    encoded_date_str, lon_min, lon_max, lat_min, lat_max, encoded_unique_acq_id = struct.unpack( _HEADER_FORMAT, packed_data)
...
def decode_unique_acquisition_id_string( unique_acquisition_id_string , output_format = 'numeric'):
    ...
    if output_format == 'string':
        ...
        raise ValueError('Input output_format = "{}"  not valid, it should be "numeric" or "string"'.format(output_format) )
...
def write_chains_configuration_file( configuration_params, configuration_file_xml ):
    ...
    if chain_id == 'AGB' or chain_id == 'FD':
        ...
        if not ModelBasedEqualization.text == '1' and not ModelBasedEqualization.text == '2' and not ModelBasedEqualization.text == '3':
            error_str = 'Configuration flag "ModelBasedEqualization" value "{}" not valid, choose among "1", "2" or "3", where "1"->always OFF; "2"->always ON; "3"->ON only if two acquisitions are present'.format(ModelBasedEqualization.text)
...
def parse_biomassL2_main_input_file(input_file_xml ):
    ...
    for date_UTC_Item in L1cDates:
        ...
        if 'start' in attrib:
            ...
            logging.error( error_message )
            raise ValueError( 'Main input file: L1cDate "value" attribute should be "start" or "stop": "'+attrib+'" is not supported' )
    ...
    main_input_struct = main_input_params( L1c_repository,
                                           L1c_aux_data_repository,
                                           proc_flags_struct,
                                           L1c_date_struct,
...
def write_fd_lookup_table( lookup_table_file_name_xml, table_type, lut_dict ):
    ...
    if not table_type == 'fnf' and not table_type == 'covariance':
        raise ValueError('#3th iinput should be a string of value "fnf" or "covariance": "'+table_type+'" is not recognized')
...
def parse_chains_input_file( input_file_xml ):
    ...
    if len(L1cStack_Items) != number_of_stacks:
        error_message = chain_id+' Input File: the number of L1cStack elements should be equal to the number of L1cProduct elements'
        logging.error( error_message )
        raise ValueError( error_message )
    ...
    for L1cStack_Item in L1cStack_Items:
        ...
        if not stack_id in stack_composition.keys():
            error_message = chain_id+' Input File: the "stack_id" values of L1cStack elelemts does not match the "stack_id" values of L1cProduct elements'
            logging.error( error_message )
            raise ValueError( error_message )
...
def check_chains_input_file( proc_inputs ):
    ...
    if chain_id == 'AGB':
        if not os.path.exists( proc_inputs.reference_agb_folder ):
            error_message = ' Input file AuxiliaryProductList ReferenceAGB does not exist: '+proc_inputs.reference_agb_folder
            logging.error( error_message )
            raise RuntimeError ( error_message  ) 
    if chain_id == 'FD':
        if not os.path.exists( proc_inputs.average_covariance_folder ):
            error_message = ' Input  AuxiliaryProductList AverageCovarianceFolder does not exist: '+proc_inputs.average_covariance_folder
            logging.error( error_message )
            raise RuntimeError ( error_message  ) 
    if chain_id == 'FH':
        if not os.path.exists( proc_inputs.system_decorrelation_fun_folder ):
            error_message = ' Input file AuxiliaryProductList SystemDecorrelationFunction does not exist: '+proc_inputs.system_decorrelation_fun_folder
            logging.error( error_message )
            raise RuntimeError ( error_message ) 

    if not os.path.exists( proc_inputs.forest_mask_catalogue_folder ):
        error_message = ' Input file AuxiliaryProductList forest non forest mask catalogue folder does not exist: '+proc_inputs.forest_mask_catalogue_folder
        logging.error( error_message )
        raise RuntimeError ( error_message  ) 
    
    if len(proc_inputs.geographic_grid_sampling)==0 or proc_inputs.geographic_grid_sampling<=0:
        error_message = ' Input file GeographicGridSampling should be a positive number: '+proc_inputs.geographic_grid_sampling
        logging.error( error_message )
        raise RuntimeError ( error_message  ) 
        
    if not os.path.exists( proc_inputs.forest_mask_catalogue_folder ):
        error_message = ' Input file AuxiliaryProductList ForestMask does not exist: '+proc_inputs.forest_mask_catalogue_folder 
        logging.error( error_message )
        raise RuntimeError ( error_message ) 
...
def parse_chains_configuration_file( configuration_file_xml ):
    ...
    if chain_id == 'AGB' or chain_id == 'FD':
        ...
        if not equalization_flag == '1' and not equalization_flag == '2' and not equalization_flag == '3':
            error_str = 'Configuration flag "ModelBasedEqualization" value "{}" not valid, choose among "1", "2" or "3", where "1"->always OFF; "2"->always ON; "3"->ON only if two acquisitions are present'.format(equalization_flag)
            logging.error(error_str)
            raise
    ...
    if chain_id == 'AGB':
        ...
        agb_model_scaling_limits   = [ float( parameter_ranges_Item.find('agb_model_scaling')  .find('min').text ), float( parameter_ranges_Item.find('agb_model_scaling')  .find('max').text ) ]
        agb_model_exponent_limits  = [ float( parameter_ranges_Item.find('agb_model_exponent') .find('min').text ), float( parameter_ranges_Item.find('agb_model_exponent') .find('max').text ) ]
        agb_cosine_exponent_limits = [ float( parameter_ranges_Item.find('agb_cosine_exponent').find('min').text ), float( parameter_ranges_Item.find('agb_cosine_exponent').find('max').text ) ]
        agb_estimation_limits      = [ float( parameter_ranges_Item.find('agb_estimation')     .find('min').text ), float( parameter_ranges_Item.find('agb_estimation').find('max').text ) ]
        ...
        estimation_valid_values_limits = [float( chain_field_Item.find('EstimationValidValuesLimits').find('min').text) , float( chain_field_Item.find('EstimationValidValuesLimits').find('max').text)]
        
        
        model_parameters = agb_model_params(agb_model_scaling_limits, agb_model_exponent_limits, agb_cosine_exponent_limits, agb_estimation_limits, \
                                                   changes_across_pol, changes_across_stacks, changes_across_global_cycle, changes_across_heading, \
                                                   changes_across_swath, changes_across_subswath, changes_across_azimuth)
        
        AGB = agb_est_params( number_of_tests, fraction_of_roi_per_test, fraction_of_cal_per_test, intermediate_ground_averaging, product_resolution, distance_sampling_area, parameter_block_size, \
                              distance_parameter_block, \
                              min_number_of_rois, min_number_of_rois_per_stack, min_number_of_cals_per_test, min_number_of_rois_per_test,\
                              model_parameters, estimation_valid_values_limits)
        ...
        kz_thresholds = [float( chain_field_Item.find('KZ_thresholds').find('min').text) , float( chain_field_Item.find('KZ_thresholds').find('max').text)]
        ...
        if median_factor_temp-median_factor>0:
            logging.warning('FH configuration file MedianFactor should be integer, the value that will be used is: {}'.format(median_factor))
        ...
        number_of_temporal_decorrelation_value = int( model_parameters_Item.find('NumberOfTemporalDecorrelationValue').text )
        if maximum_height<2 or number_of_extinction_value<2 or number_of_ground_volume_ratio_value<2 or number_of_temporal_decorrelation_value<2:
            ...
            logging.error( error_message )
            raise ValueError( error_message )
        model_parameters = FH_model_params( estimation_valid_values_limits, maximum_height, number_of_extinction_value, number_of_ground_volume_ratio_value, number_of_temporal_decorrelation_value)
        ...
        TOMO_FH                     = TOMO_FH_est_params( estimation_valid_values_limits, product_resolution, enable_super_resolution, regularization_noise_factor, power_threshold, median_factor)
...
def parse_campaign_params_simple(campaign_params_file_xml, tag, campaign_tag):
    ...
    date_UTC = date[0][:2]+'-'+month_str+'-'+date[0][6:10]+' '+date[0][11:13]+':'+date[0][14:17]+':00.000000000000'
