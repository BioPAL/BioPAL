            ...
            if not isinstance(arg, int):
                if isinstance(arg, float):
                    ...
                    try:
                        ...
                        raise ValueError("{} wrong type: {} != int".format(tag, type(arg)))
            if arg < 0:
...
class AttitudeInfo(MetaDataElement):
    ...
    def reference_time(self, reference_time):
        if not (isinstance(reference_time, int) or isinstance(reference_time, float) or isinstance(reference_time,
                                                                                                   PreciseDateTime)):
...
class _Poly2DVector(MetaDataElement):
    ...
    def get_poly(self, index):
        if index < self.get_number_of_poly() and index >= 0:
...
class DataStatistics(MetaDataElement):
    ...
    def get_data_block_statistic(self, index):
        if index < self.get_number_of_data_block_statistic() and index >= 0:
...
class MetaDataChannel:
    ...
    def __init__(self):
        ...
        for element_tag in self._supported_elements:
            ...
            setattr(self, element_tag, property(lambda s: self.get_element(element_tag)))
