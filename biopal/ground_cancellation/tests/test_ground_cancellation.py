...
class TestGroundCancellation:
    ...
    def test_ground_cancellation(self):
        ...
        assert ("HH" in pols and "HV" in pols and "VV" in pols) and len(
            pols
        ) == 3, "wrong output number of polarizations, in test with space varying OFF"
        ...
        for pol, data in notched_pol_dict.items():
            ...
            if multi_master_flag:
                assert isinstance(
                    data[0, 0], np.float64
                ), "wrong output data type; when multi_master_flag is true it is supposed to be float64, in test with space varying OFF"
            else:
                assert isinstance(
                    data[0, 0], np.complex128
                ), "wrong output data type; when multi_master_flag is false it is supposed to be complex128, in test with space varying OFF"
    ...
    def test_ground_cancellation_space_varying(self):
        ...
        assert ("HH" in pols and "HV" in pols and "VV" in pols) and len(
            pols
        ) == 3, "wrong output number of polarizations, in test with space varying ON"
        ...
        for pol, data in notched_pol_dict.items():
            ...
            if multi_master_flag:
                assert isinstance(
                    data[0, 0], np.float64
                ), "wrong output data type; when multi_master_flag is true it is supposed to be float64, in test with space varying ON"
            else:
                assert isinstance(
                    data[0, 0], np.complex128
                ), "wrong output data type; when multi_master_flag is false it is supposed to be complex128, in test with space varying ON"
    ...
    def initial_values(self):
        ...
        wave_number_dict = {
            "BSL_00": 0 * wave_number_dummy,
            "BSL_01": -0.03 * wave_number_dummy,
            "BSL_02": 0.08 * wave_number_dummy,
        }
