...
from biopal.io.data_io import (read_auxiliary_multi_channels,
                               read_auxiliary_single_channel, read_data,
                               read_ecef_grid, readBiomassHeader,
                               tandemx_fnf_read, tiff_formatter)
...
def mosaiking_fnf(equi7_main_full_folder, mosaiking_out_folder):
    # equi7_main_full_folder is the folder containing the temporary equi7 generated by the processor:
    # the final mosaiking will prodice the output folder, containing only Equi7 tiles, with no mention to stack IDs
    # this function uses listdir to retrive files and folder names from equi7_main_full_folder
    ...
    # Each tile in the input equi7_main_full_folder contains one tiff with three layers:
    ...
    for equi7_tile_name in equi7_tile_names:
        ...
        for equi7_tiff_name in equi7_tile_tiff_full_names:
            ...
            # read current tiff to be mosaiked:
            input_image_driver = gdal.Open(equi7_tiff_name, GA_ReadOnly)
            loaded_equi7_tile_pforest = input_image_driver.GetRasterBand(pforest_layer_index).ReadAsArray()
            loaded_equi7_tile_quality = input_image_driver.GetRasterBand(quality_layer_index).ReadAsArray()
            loaded_equi7_tile_fnf = input_image_driver.GetRasterBand(fnf_layer_index).ReadAsArray()

            # mosaiking (mean where both, use une where just one) it with previous data
            if not "out_mosaiked_pforest" in locals():
                out_mosaiked_pforest = loaded_equi7_tile_pforest
                out_mosaiked_quality = loaded_equi7_tile_quality
                out_mosaiked_fnf = loaded_equi7_tile_fnf
            else:
                where_loaded_only_is_not_zero = np.logical_and(out_mosaiked_pforest == 0.0, loaded_equi7_tile_pforest != 0.0,)
                where_both_are_not_zero = np.logical_and(out_mosaiked_pforest != 0.0,loaded_equi7_tile_pforest != 0.0,)

                # pforest
                out_mosaiked_pforest[where_loaded_only_is_not_zero] = loaded_equi7_tile_pforest[where_loaded_only_is_not_zero]
                out_mosaiked_pforest[where_both_are_not_zero] = (
                    loaded_equi7_tile_pforest[where_both_are_not_zero] + out_mosaiked_pforest[where_both_are_not_zero]
                ) / 2
                ...
                # fnf
                out_mosaiked_fnf[where_loaded_only_is_not_zero] = loaded_equi7_tile_fnf[
                    where_loaded_only_is_not_zero
                ]
                out_mosaiked_fnf[where_both_are_not_zero] = (
                    loaded_equi7_tile_fnf[where_both_are_not_zero] + out_mosaiked_fnf[where_both_are_not_zero]
                ) / 2

        # write the output final mosaiked tiff of the current equi7 tile:
        out_mosaiked_equi7_name_pforest = os.path.join(out_tile_equi7_full_folder, "probability_of_forest.tif")
        out_mosaiked_equi7_name_quality = os.path.join(out_tile_equi7_full_folder, "quality.tif")
        out_mosaiked_equi7_name_fnf = os.path.join(out_tile_equi7_full_folder, "FNF.tif")

        geotransform = input_image_driver.GetGeoTransform()
        projection = input_image_driver.GetProjection()

        # write three separated tiff
        os.makedirs(os.path.dirname(out_mosaiked_equi7_name_pforest))
        Nx, Ny = out_mosaiked_pforest.shape

        # pforest and quality are saved as float (values from 0.0 to 1.0, with special -9999.0 for no data)
        driver_1 = gdal.GetDriverByName("GTiff")  
        outdata = driver_1.Create(out_mosaiked_equi7_name_pforest, Ny, Nx, 2, gdal.GDT_Float32)
        outdata.SetGeoTransform(geotransform)
        outdata.SetProjection(projection)
        outdata.GetRasterBand(1).WriteArray(out_mosaiked_pforest)
        outdata.FlushCache()  # saves to disk
        outdata = None
        driver_1 = None

        driver_2 = gdal.GetDriverByName("GTiff")
        outdata = driver_2.Create(out_mosaiked_equi7_name_quality, Ny, Nx, 2, gdal.GDT_Float32)
        outdata.SetGeoTransform(geotransform)
        outdata.SetProjection(projection)
        outdata.GetRasterBand(1).WriteArray(out_mosaiked_quality)
        outdata.FlushCache()  # saves to disk
        outdata = None
        driver_2 = None

        # FNF is saved as binary (three integers, where 0=Flase, 1=True, 255 = no data)
        driver_3 = gdal.GetDriverByName("GTiff")
        outdata = driver_3.Create(out_mosaiked_equi7_name_fnf, Ny, Nx, 2, gdal.GDT_Byte)
        outdata.SetGeoTransform(geotransform)
        outdata.SetProjection(projection)
        outdata.GetRasterBand(1).WriteArray(np.uint8(out_mosaiked_fnf))
        outdata.FlushCache()  # saves to disk
        outdata = None
        driver_3 = None

        del out_mosaiked_pforest, out_mosaiked_quality, out_mosaiked_fnf
