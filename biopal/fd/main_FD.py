    ...
    def _run(self, input_file_xml):
        ...
        for time_step_idx, (global_cycle_idx, uniqie_acq_ids_all_cycles_list) in enumerate( self.global_cycle_dict.items() ):
            ...
            try:
                ...
                read_and_oversample_data( self.proc_inputs.L1c_repository, uniqie_acq_ids_all_cycles_list, self.proc_conf.enable_resampling )
            ...
            try:
                ...
                geometry_aux_are_present = check_if_geometry_auxiliaries_are_present(self.proc_inputs, unique_stack_id, uniqie_acq_ids_all_cycles_list, read_ref_h = read_ref_h)
                ...
                if self.proc_conf.compute_geometry or not geometry_aux_are_present:
                    ...
                    compute_and_oversample_geometry_auxiliaries(self.proc_inputs.L1c_repository, self.proc_inputs, unique_stack_id, uniqie_acq_ids_all_cycles_list, master_id, self.proc_conf.enable_resampling, force_ellipsoid=True )                   
                                              
                    ecef_grid, off_nadir_angle_rad, slope, kz, reference_height, _, _= \
                    compute_and_oversample_geometry_auxiliaries(self.proc_inputs.L1c_repository, self.proc_inputs, unique_stack_id, uniqie_acq_ids_all_cycles_list, master_id, self.proc_conf.enable_resampling, sar_geometry_master=sar_geometry_master)
                    ...
                    read_and_oversample_aux_data( self.proc_inputs, unique_stack_id, uniqie_acq_ids_all_cycles_list, self.proc_conf.enable_resampling, raster_info_orig )
                ...
                if read_cal_screens:
                    ...
                    read_and_oversample_aux_data( self.proc_inputs, unique_stack_id, uniqie_acq_ids_all_cycles_list, self.proc_conf.enable_resampling, raster_info_orig, read_cal_screens = read_cal_screens,\
            ...
            try:
                ...
                beta0_notched = ground_cancellation(beta0_calibrated, kz, self.proc_conf.ground_cancellation.multi_master_flag, self.proc_conf.ground_cancellation.enhanced_forest_height, \
                                self.proc_conf.ground_cancellation.equalization_flag, raster_info.resolution_m_slant_rg, off_nadir_angle_rad[master_id], slope) # beta0_calibrated: 3 pol (nrg x Naz  x Nimmagini); beta0_notched: 3 pol (Nrg x Naz x 1 immagine) 
            ...
            if self.stack_idx == 0 and time_step_idx == 0:
                ...
                if fnf_format == 'TANDEM-X':
                    ...
                    equi7_initial_fnf_mask_fnames = fnf_equi7_load_filter_equi7format( self.proc_inputs.forest_mask_catalogue_folder, e7g, self.proc_conf.FD.product_resolution, temp_output_folder, self.gdal_path)
            ...
            for equi7_tile_idx  in np.arange( number_of_equi7_tiles ):
                ...
                if time_step_idx == 0:
                    ...
                    equi7_avg_cov_out_tiff_name_prev = os.path.join( self.proc_inputs.average_covariance_folder, unique_stack_id_prev, equi7_zone_name, equi7_tile_name, equi7_cov_tiff_name_curr )
        ...
        equi7_zone_fnf_mask_outdir     = os.path.join( products_folder, 'global_FD', 'FNF_Mask'   , self.nominal_geometry_stack_id, equi7_zone_name )
        ...
        for equi7_tile_idx  in np.arange( number_of_equi7_tiles ):    
            ...
            final_disturbance_matrix_curr = np.logical_xor( initial_forest_mask_curr, self.final_forest_mask_data[equi7_tile_name] )
