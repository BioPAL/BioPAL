...
from biopal.fd.processing_FD import (
    alg_wishart_SU,
)
...
"""
    ...
    def _run(self, input_file_xml):
        ...
        # Main APP #2: FD Core Processing
        # agb_processing_obj = CoreProcessingFD(
...
class StackBasedProcessingFD(Task):
    ...
    def __init__(
        self, configuration_file_xml, geographic_boundaries, gdal_path=None,
    ):
    ...
    def _run(self, input_file_xml):
        ...
        for idx, (unique_stack_id, unique_acq_pf_names) in enumerate( proc_inputs.stack_composition.items() ): 
            nominal_geometry_stack_id =     unique_stack_id[6: ] # remove the global cycle indication, maintain the geometry indications
        ...
        for stack_idx, (nominal_geometry_stack_id, global_cycle_dict) in enumerate( cycles_composition.items() ):     
            ...
            _, heading_deg, rg_swath_idx, rg_sub_swath_idx, az_swath_idx, _ = decode_unique_acquisition_id_string( next(iter(global_cycle_dict.values()))[0] )
            ...
            for time_step_idx, (global_cycle_idx, uniqie_acq_ids_all_cycles_list) in enumerate( global_cycle_dict.items() ):
                ...
                time_tag_mjd_curr = get_data_time_stamp( proc_inputs.L1c_repository, uniqie_acq_ids_all_cycles_list[0] )
                ...
                if time_step_idx == 0:
                    ...
                    logging.debug('time_step_idx {}, global_cycle_idx_prev_str {} '.format(time_step_idx, global_cycle_idx_prev_str))
                ...
                logging.info('FD: disturbance computation step #{} of #{}; current global cycle is '.format( time_step_idx+1, number_of_global_cycles)+'"GC_'+global_cycle_idx_str+'"'+' current geometry is '+nominal_geometry_stack_id )
                ...
                try:
                    ...
                    read_and_oversample_data( proc_inputs.L1c_repository, uniqie_acq_ids_all_cycles_list, proc_conf.enable_resampling )
                ...
                try:
                    ...
                    geometry_aux_are_present = check_if_geometry_auxiliaries_are_present(proc_inputs, unique_stack_id, uniqie_acq_ids_all_cycles_list, read_ref_h = read_ref_h)
                    ...
                    if proc_conf.compute_geometry or not geometry_aux_are_present:
                        ...
                        if proc_conf.compute_geometry:
                            ...
                            logging.warning( 'FD: calling geometry library since AuxiliaryProductsFolder "Geometry" is empty or not complete \n')
                            
    
                        _, _, ellipsoid_slope, _, _, _, sar_geometry_master = \
                        compute_and_oversample_geometry_auxiliaries(proc_inputs.L1c_repository, proc_inputs, unique_stack_id, uniqie_acq_ids_all_cycles_list, master_id, proc_conf.enable_resampling, force_ellipsoid=True )                   
                                                  
                        ecef_grid, off_nadir_angle_rad, slope, kz, reference_height, _, _= \
                        compute_and_oversample_geometry_auxiliaries(proc_inputs.L1c_repository, proc_inputs, unique_stack_id, uniqie_acq_ids_all_cycles_list, master_id, proc_conf.enable_resampling, sar_geometry_master=sar_geometry_master)
                        ...
                        logging.info( 'FD: geometry auxiliaries are provided from user, so they are now being loaded and not computed, for stack '+unique_stack_id+'\n')
                       
                        ecef_grid, off_nadir_angle_rad, slope,  kz, reference_height, _, _, _, _, _, _, _ = \
                        read_and_oversample_aux_data( proc_inputs, unique_stack_id, uniqie_acq_ids_all_cycles_list, proc_conf.enable_resampling, raster_info_orig )
                    ...
                    if read_cal_screens:
                        ...
                        read_and_oversample_aux_data( proc_inputs, unique_stack_id, uniqie_acq_ids_all_cycles_list, proc_conf.enable_resampling, raster_info_orig, read_cal_screens = read_cal_screens,\
                ...
                try:
                    ...
                    beta0_notched = ground_cancellation(beta0_calibrated, kz, proc_conf.ground_cancellation.multi_master_flag, proc_conf.ground_cancellation.enhanced_forest_height, \
                                    proc_conf.ground_cancellation.equalization_flag, raster_info.resolution_m_slant_rg, off_nadir_angle_rad[master_id], slope) # beta0_calibrated: 3 pol (nrg x Naz  x Nimmagini); beta0_notched: 3 pol (Nrg x Naz x 1 immagine) 
                ...
                cov_est_window_size = proc_conf.FD.product_resolution
                
                if proc_conf.multilook_heading_correction:
                    _, heading_deg, _, _, _, _ = \
                ...
                try: 
                    ...
                    MPMB_covariance_sr = covariance_matrix_mat2vec( MPMB_covariance_sr ) #MPMB_covariance isreshaped from 3x3xNrgxNaz to 6xNrgxNaz
                ...
                try:
                    ...
                    inc_angle_ground_rad = geocoding( (off_nadir_angle_rad[master_id][rg_vec_subs,:][:,az_vec_subs]- slope[rg_vec_subs,:][:,az_vec_subs] ) , \
                                                       lon_in, lat_in, lonMeshed_out, latMeshed_out, valid_values_mask )
                    logging.info('...done.\n')
                ...
                try:
                    ...
                    tiff_formatter( MPMB_gr_temp_npy_fnames, cov_ground_fname, geotransform, gdal_data_format=gdal.GDT_CFloat32, multi_layers_tiff=True, time_tag= str( time_tag_mjd_curr ) )
                    
                    # incidence angle matrix geotiff formatting:
                    tiff_formatter( inc_angle_ground_rad, inc_angle_ground_fname, geotransform, gdal_data_format=gdal.GDT_Float32, multi_layers_tiff=False, time_tag= str( time_tag_mjd_curr ) )
                except Exception as e:
                ...
                if stack_idx == 0 and time_step_idx == 0:
                    ...
                    if fnf_format == 'TANDEM-X':
                        ...
                        equi7_initial_fnf_mask_fnames = fnf_tandemx_load_filter_equi7format ( proc_inputs.forest_mask_catalogue_folder, e7g, proc_conf.FD.product_resolution, temp_output_folder, self.gdal_path, self.geographic_boundaries, time_tag_mjd_initial )
                        ...
                        equi7_initial_fnf_mask_fnames = fnf_equi7_load_filter_equi7format( proc_inputs.forest_mask_catalogue_folder, e7g, proc_conf.FD.product_resolution, temp_output_folder, self.gdal_path)
                ...
                equi7_zone_avg_cov_outdir      = os.path.join( products_folder, 'global_FD', 'AverageCovariance', unique_stack_id      , equi7_zone_name )
                equi7_zone_avg_cov_outdir_prev = os.path.join( products_folder, 'global_FD', 'AverageCovariance', unique_stack_id_prev , equi7_zone_name )
                ...
                for equi7_tile_idx  in np.arange( number_of_equi7_tiles ):
                    ...
                    equi7_tile_name = os.path.basename( os.path.dirname( equi7_cov_temp_fnames[equi7_tile_idx] ) ) # current equi7 tile name inside the equi7 zone
                    ...
                    if not equi7_tile_name in final_forest_mask_data.keys():
                        ...
                        equi7_initial_fnf_mask_fname_curr = [name for name in equi7_initial_fnf_mask_fnames if equi7_tile_name in name ][0]
                    ...
                    mat_number_of_looks = np.floor ( (proc_conf.FD.product_resolution**2) / (raster_info.resolution_m_az * raster_info.resolution_m_slant_rg/np.sin( inc_angle_equi7_rad ) ) )         
                    ...
                    if time_step_idx == 0:
                        ...
                        equi7_avg_cov_out_tiff_name_prev = os.path.join( proc_inputs.average_covariance_folder, unique_stack_id_prev, equi7_zone_name, equi7_tile_name, equi7_cov_tiff_name_curr )
                    ...
                    if time_step_idx == 0 and not os.path.exists( equi7_avg_cov_out_tiff_name_prev ):
                        ...
                        if prev_global_cycle_id<0:
                            logging.info('FD first iteration, since first Global Cycle in input data is "0", cannot retrieve any previous Global Cycle Average Covariance' )
                        else:
                            logging.info('FD first iteration, input Average Covariance tiff not present at: {}'.format( equi7_avg_cov_out_tiff_name_prev ))
                        
                        logging.info('Saving current computed covariance as first Average Covariance (cannot compute disturbance at first iteration without input Average Covariance )')
                    ...
                    for rg_idx in np.arange(Nrg_equi7):
                        ...
                        for az_idx in np.arange( Naz_equi7 ):
                            ...
                            Y_N, j_idx, disturbance_matrix[rg_idx, az_idx], disturbance_prob_matrix[rg_idx, az_idx], _ = alg_wishart_SU( X_i, Y_N, j_idx, number_of_pols, cov_number_of_looks, proc_conf.FD.confidence_level, False)
                    ...
                    tiff_formatter( \
                    [ Y_N_6x_vec[0,:,:],Y_N_6x_vec[1,:,:],Y_N_6x_vec[2,:,:],Y_N_6x_vec[3,:,:], Y_N_6x_vec[4,:,:],Y_N_6x_vec[5,:,:] ], \
                    equi7_COV_out_fname, geotransform_out, gdal_data_format=gdal.GDT_Float32, projection=projection_equi7, multi_layers_tiff=True, time_tag= str( time_tag_mjd_curr)  ) 
     
                    logging.info('FD: equi7 tile {} of {}'.format(equi7_tile_idx, len(equi7_cov_temp_fnames) )+' average covariange processing done.\n' )
                    
    
                # keep track of the unique_stack_id for next iteration
                unique_stack_id_prev = unique_stack_id
    
                logging.info('FD: disturbance computation for global cycle "GC_'+global_cycle_idx_str+'" of geometry '+nominal_geometry_stack_id+' done.\n' )
...
#def CoreProcessingFD():
        ########################## NOT STACK BASED STEPS ##############################
            ...
            equi7_zone_disturbance_outdir  = os.path.join( products_folder, 'global_FD', 'Disturbance', nominal_geometry_stack_id, equi7_zone_name )
            equi7_zone_fnf_mask_outdir     = os.path.join( products_folder, 'global_FD', 'FNF_Mask'   , nominal_geometry_stack_id, equi7_zone_name )
            ...
            for equi7_tile_idx  in np.arange( number_of_equi7_tiles ):    
                ...
                logging.info('FD: final step, computation of disturbance product, for equi7 tile #{} of #{}'.format(equi7_tile_idx+1, number_of_equi7_tiles))
                ...
                final_disturbance_matrix_curr = np.logical_xor( initial_forest_mask_curr, final_forest_mask_data[equi7_tile_name] )
                
                
                tiff_formatter( final_disturbance_matrix_curr,       equi7_disturbance_out_fname, geotransform_out, gdal_data_format=gdal.GDT_Float32, projection=projection_equi7, time_tag= str( time_tag_mjd_final  ))
               
                logging.info('FD: final step,saving the final fnf mask for equi7 tile #{} of #{}'.format(equi7_tile_idx, number_of_equi7_tiles))
                tiff_formatter( final_forest_mask_data[equi7_tile_name], equi7_fnf_mask_outfname, geotransform_out, gdal_data_format=gdal.GDT_Float32, projection=projection_equi7 , time_tag= str( time_tag_mjd_final ) )
